# Исходный код: https://github.com/devtud/smppy
...
class UnknownUserException(Exception):
    "Пользователь не в списке обрабатываемых юзеров"
    pass
...
class SmppProtocol(asyncio.Protocol, abc.ABC):
    ...
    def connection_made(self, transport: asyncio.Transport) -> None:
        ...
        # self.app.logger.info(f"Data by: {ip}")
        self._transport = transport
        self._ref_num = 0

    def connection_lost(self, exc: Optional[Exception]) -> None:
        ip = self._transport.get_extra_info('peername')[0]
        if exc:
            self.app.logger.error(f'ERROR: {exc}')
        else:
            self.app.logger.info(f'Closing connection ({ip})')
    ...
    async def send_deliver_sm(self,
        ...
        self.app.logger.debug(f'Send deliver')
    ...
    async def on_deliver_sm_resp(self, request: DeliverSMResp):
        encoder = PDUEncoder()
        self._transport.write(encoder.encode(request))
        pass
    ...
    def md5_hash(self, data: str) -> str:
    ...
    async def handle_data_received(self, data: bytes):
        ...
        ip = self._transport.get_extra_info('peername')[0]
        self.app.logger.debug(f'command received: {pdu.command_id} by ({ip})')
        
        if pdu.command_id == CommandId.submit_sm:
            self.app.logger.debug(f'message PDU params: {pdu.params} ({ip})')
            submit_sm = SubmitSM(sequence_number=pdu.sequence_number, **pdu.params)
            sms = SMStringEncoder().decode_SM(submit_sm).str
            
            message_id = self.md5_hash(sms)
            self._send_response(
                submit_sm.require_ack(sequence_number=submit_sm.sequence_number, message_id=message_id)
            )
            ...
            self.app.logger.info(f'Received "{text}" from {source_number} to {dest_number} ({ip})')
            ...
            self.app.logger.info(f'Bind transceiver ({ip})')
            ...
            if self.is_bound:
                ...
                self.app.logger.debug(f'Client try to connect {client.system_id}:{client.password} ({ip})')
               
                self._client = client
                try:
                    client = await self.app.handle_bound_client(client=client)
                except UnknownUserException:
                    self.app.logger.debug(f'Unknown user {client.system_id}:{client.password} ({ip})')
            ...
            self.app.logger.info(f'Unbind transceiver ({ip})')
            self.is_bound = False
            ...
            # Поддержание сокета
            self.app.logger.debug(f'Enquire_link ({ip})')
        #elif pdu.command_id == CommandId.deliver_sm:
        #    self.app.logger.debug(f'deliver_sm ({ip})')
        #elif pdu.command_id == CommandId.deliver_sm_resp:
        #    self.app.logger.debug(f'deliver_sm_resp ({ip})')
        else:
            self.app.logger.debug(f'Command_id {pdu.command_id} ({ip})')
